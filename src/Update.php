...
class Update implements \ArrayAccess
{
    ...
    private function parseUpdate()
    {
        ...
        $this->updateArr = $local_update_arr;
    ...
    {
        if (isset($this->updateObj->$value)) {
            return $this->updateObj->$value;
        }
        if (isset($this->updateObj->{$this->updateType}->$value)) {
            return $this->updateObj->{$this->updateType}->$value;
        }
        // in cbq update the message is inside cbq object
        if (isset($this->updateObj->{$this->updateType}->message->$value)) {
            return $this->updateObj->{$this->updateType}->message->$value;
        }
    }
    ...
    public function edit($newMessage, $replyMarkup = null, $ent = null)
    {
        if (isset($this->update)) {
            if (!$this->service) {
                if ($this->media != null) {
                    return $this->bot->editMessageCaption($this->chat->id, $this->message_id, $this->inline_message_id, $newMessage, $replyMarkup, entities: $ent);
                } else {
                    return $this->bot->editMessageText($this->chat->id, $this->message_id, $this->inline_message_id, $newMessage, $replyMarkup, entities: $ent);
                }
            } else {
                return $this;
            }
    ...
    public function pin($dis_notification = null)
    {
        if (isset($this->update)) {
            if (!$this->service) {
                return $this->bot->pinChatMessage($this->chat->id, $this->message_id, $dis_notification);
            } else {
                return $this;
            }
        }
    ...
    public function forward($to, $noCredit = false, $rm = null, $replyTo = null, $caption = null, $ent = null, $thread = null)
    {
        if (isset($this->update) && !$this->has_protected_content) {
            if (!$this->service) {
                if ($noCredit) {
                    return $this->bot->copyMessage($to, $this->chat->id, $this->message_id, $rm, $replyTo, $caption, $ent, $thread);
                } else {
                    return $this->bot->forwardMessage($to, $this->chat->id, $this->message_id, $thread);
                }
            } else {
                return $this;
            }
    ...
    {
        if (isset($this->update)) {
            return $this->bot->sendMessage($this->chat->id ?? $this->from->id, $text, $replyMarkup, $this->message_id ?? null, entities: $ent, threadId: $this->thread);
        }
    }
    ...
    {
        if (isset($this->update)) {
            return $this->bot->getChatMember($id ?? $this->chat->id, $this->from->id);
        }
    }
    ...
    {
        if (isset($this->update)) {
            return $this->bot->editMessageReplyMarkup($this->chat->id, $this->message_id, $this->inline_message_id, $newKeyboard);
        }
    }
    ...
    public function alert($text, $show = false)
    {
        if (isset($this->update)) {
            if ($this->data != null) {
                return $this->bot->answerCallbackQUery($this->id, $text, $show);
            } else {
                return $this;
            }
        }
    ...
    public function ban($id = null, $removeMessages = false)
    {
        if ($this->chatType != 'private') {
            if ($this->sender_chat == null) {
                return $this->bot->banChatMember($this->chat->id, $id ?? $this->from->id, deleteAllMessages: $removeMessages);
            } else {
                return $this->bot->banChatSenderChat($this->chat->id, $id ?? $this->from->id);
            }
        } else {
            return $this;
        }
